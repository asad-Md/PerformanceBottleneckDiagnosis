 ...
 * reader.c
 *
 * Opens the four pinned maps from /sys/fs/bpf/ (written by the BPF
 * programs while pinner.c was running), reads all entries, merges
 * them by (pid, cpu), and writes one CSV row per (pid, cpu).
 *
 * No BPF loading, no attaching — pure map read.
 * The maps contain cumulative totals since pinner started.
 *
 * Build:
 *   gcc -O2 -o reader reader.c -lbpf -lelf -lz
 *
 * Run (after stopping pinner):
...
// ── drain a map into key/value arrays ─────────────────────────────────────────
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (!strcmp(argv[i], "--out")    && i+1 < argc) csv_path = argv[++i];
        else if (!strcmp(argv[i], "--append"))                append   = 1;
    }
    ...
    if (fds[0] < 0 && fds[1] < 0 && fds[2] < 0 && fds[3] < 0) {
        fprintf(stderr, "[reader] no maps found — did pinner run and pin them?\n");
        return 1;
    }
    ...
    fprintf(stderr, "[reader] entries: sched=%d mem=%d syscall=%d lock=%d\n",
            ns, nm, nsc, nl);

    // ── get current timestamp for the snapshot ────────────────────────────────
...
#define MERGE_KEYS(src_keys, src_n)                                     \
    for (int _i = 0; _i < (src_n) && nk < MAX_KEYS; _i++) {            \
        ...
        if (!_found) all_keys[nk++] = (src_keys)[_i];                   \
    }
...
#undef MERGE_KEYS
    ...
    fprintf(stderr, "[reader] %d unique (pid,cpu) entries\n", nk);
    ...
    for (int k = 0; k < nk; k++) {
        ...
        for (int i = 0; i < ns;  i++)
            if (sched_keys[i].pid == all_keys[k].pid &&
                ...
                { sv = &sched_vals[i]; if (sv->comm[0]) comm = sv->comm; break; }
        for (int i = 0; i < nm;  i++)
    ...
    fprintf(stderr, "[reader] unpinned maps\n");
