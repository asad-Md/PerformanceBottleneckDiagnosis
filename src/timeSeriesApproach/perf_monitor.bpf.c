...
int handle_sched_switch(struct trace_event_raw_sched_switch *ctx)
{
    ...
    if (!sv)
    {
        ...
        bpf_map_update_elem(&sched_map, &prev_key, &zero, BPF_NOEXIST);
    ...
    {
        struct sched_val zero = {};
        bpf_map_update_elem(&sched_map, &next_key, &zero, BPF_NOEXIST);
...
static __always_inline void sc_exit_fn(u64 id, s64 ret)
{
    ...
    if (!v)
    {
        ...
        bpf_map_update_elem(&syscall_map, &ak, &zero, BPF_NOEXIST);
...
static __always_inline void lock_exit_fn(void)
{
    ...
    if (!v)
    {
        ...
        bpf_map_update_elem(&lock_map, &ak, &zero, BPF_NOEXIST);
